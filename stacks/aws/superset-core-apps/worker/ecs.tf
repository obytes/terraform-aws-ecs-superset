module "worker_ecs" {
  ...
  desired_count                            = var.worker_ecs_params["desired_count"]
  cpu                                      = var.worker_ecs_params["cpu"]
  memory                                   = var.worker_ecs_params["memory"]
  container_name                           = var.worker_ecs_params["container_name"]
  container_port                           = var.worker_ecs_params["port"]
  service_discovery                        = var.service_discovery
  ecs_service_security_group_ingress_rules = var.worker_sg
...
[
  {
    "INSTANCE_NAME"  : "${var.worker_ecs_params["container_name"]}",
    "image": "${var.ecr_repository_url}:${var.common_tags["env"]}",
    "name": "${var.worker_ecs_params["container_name"]}",
    ...
    "portMappings": [
      {
        "containerPort": ${var.worker_ecs_params["port"]},
        "hostPort": ${var.worker_ecs_params["port"]}
      }
    ]
  }
]
